...
void loop() {
  ...
  hour_button.tick();                         // обязательная функция отработки. Должна постоянно опрашиваться
  min_button.tick();

  Serial.println(FOUR_SECOND);

  if (min_button.isStep()) {                      // если кнопка была удержана
    long long time_now = millis();                // фиксируем время
    bool is_button_presed = false;                // фиксируем, что кнопка еще не нажата после удержания
    minute = 0;
    while (millis() - time_now < FOUR_SECOND) {    // пока действует режим установки
      if (is_button_presed) {                      // если кнопка нажата в режиме установки
        time_now = millis();
        while (millis() - time_now < FOUR_SECOND) {
          min_button.tick();
          if (min_button.isPress() || min_button.isStep()) {   // если кнопка нажата или удержана
            if (minute > 60)
              minute = 0;
            minute++;                                           // добавляем минуты
            time_now = millis();                                // чтобы режим установки не закончился во время установки
          }
          for (size_t i = 0; i < num_leds_minute; i++)
            digitalWrite(leds_minute[num_leds_minute - i - 1], bitRead(minute, i));   // ставим установленное кол-во минут на светодиоды
        }
          for (size_t i = 0; i < num_leds_minute; i++)    // мигание светодиодов для обозначения пользователю режима установки
          ...
          for (size_t i = 0; i < num_leds_minute; i++)
            digitalWrite(leds_minute[i], LOW);
          watch.settime(0, minute);                        // устанавливаем новое время в RTC модуль
      }
      for (size_t i = 0; i < num_leds_minute && !is_button_presed; i++) {
        min_button.tick();
        if (min_button.isSingle())                                     // если кнопка для минут нажата один раз
          is_button_presed = true;                                     // фиксируем нажатие
        digitalWrite(leds_minute[i], HIGH);
        delay(50);
      }
      for (size_t i = 0; i < num_leds_minute && !is_button_presed; i++) {
...
//  if (minute != String(watch.gettime("i")).toInt()) {
    ...
    hour = String(watch.gettime("h")).toInt();   // получаем кол-во часов и минут из RTC  в формате строки-шаблона
    minute = String(watch.gettime("i")).toInt();   //  и конвертируем в целочисленный формат
